...
private:
    ...
    {
        int num_bits = log2(blocksize);
        address = address >> num_bits;
        int index = address % numblocks;
        int tag = address >> int(log2(numblocks));
...
public:
    ...
    lrucache(int _cachesize, int _blocksize) : cachesize(_cachesize), blocksize(_blocksize)
    {
        ...
        miss_rate = 0;
        for (auto &b : c)
        { // Use a reference to modify the actual cache blocks 
            b.valid = false;
            b.tag=-1;
            b.index=-1;
        }
    ...
    bool access(int address)
    {
        ...
        int index = a.second;
        int tag = a.first;
        //cout<<index<<" i"<<tag<<" fes"<<endl;
        if (c[index].tag == tag && c[index].valid)
        {
            //cout<<"t"<<endl;
        ...
        {   
            ...
            c[index].valid = true;
            lrulist.push_front(index);
            if (lrulist.size() > numblocks)
            {
                int lruindex = lrulist.back();
                lrulist.pop_back();
                c[lruindex].valid = false;
            }
            return false;
        }
    }
...
public:
    ...
    void Fetch(core &core, string memory[], lrucache &cache)
    {   
    ...
    void rate(int address,lrucache &cache){
        if(cache.access(address)){
            cache.hit_rate++;
        }
        else cache.miss_rate++;
    }
    ...
    void detect_data_Hazard(core &core)
    {
        ...
        if (i >= 0)
        {
            ...
            if (core.Buffers[core.i2].rs1 == core.Buffers[i].rd || core.Buffers[core.i2].rs2 == core.Buffers[i].rd || (core.Buffers[i].opcode == "lw"))
            {
                if (!core.forwarding)
                {
                    core.temp_stall = 2;
...
void processor::run()
{
    ...
    while (flag1 == 1 || flag2 == 1)
    {
        ...
        if (flag1 == 1)
        {
            ...
            pipe_line.Fetch(cores[0], memory, cache);
        ...
        if (flag2 == 1)
        {
            ...
            pipe_line.Fetch(cores[1], memory, cache);
...
void print1(const string filename, int c0_stall, int c1_stall, int c0_cycles, int c1_cycles, int c0_in, int c1_in, bool c0_fwd, bool c1_fwd, int missrate, int hitrate)
{
    ofstream outputFile(filename);
    if (!outputFile.is_open())
    {
    ...
    if (c0_fwd)
    {
        ...
        outputFile << "Without Data Forwarding\n";
    ...
    outputFile << endl;
    outputFile << "----------------------------------" << endl;
    outputFile << "Hit rate: " << hitrate << endl;
    outputFile << "Miss rate: " << missrate << endl;
    outputFile.close();
}
...
int main()
{
    ...
    print1("Cores-info-output.txt", sim.cores[0].num_stalls, sim.cores[1].num_stalls, sim.cores[0].num_Clock_Cycles, sim.cores[1].num_Clock_Cycles, sim.cores[0].num_Instructions, sim.cores[1].num_Instructions, sim.cores[0].forwarding, sim.cores[1].forwarding, sim.cache.miss_rate, sim.cache.hit_rate);
    // For printing the content present in the memory
