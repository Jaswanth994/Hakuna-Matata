...
public:
    ...
    void Fetch(core &core, string memory[],cache &cache)
    {
        ...
        {
            // cout<<core.pc<<"b"<<endl;
            lrurate(core.pc, cache);
            mrurate(core.pc, cache);
        }
    ...
    void mrurate(int address, cache &cache)
    {
        if (cache.mruaccess(address))
        {
            cache.mruhit++;
        }
        else
...
void print1(const string filename, int c0_stall, int c1_stall, int c0_cycles, int c1_cycles, int c0_in, int c1_in, bool c0_fwd, bool c1_fwd, int lrumiss, int lruhit, int mruhit, int mrumiss)
{
    ofstream outputFile(filename);
    if (!outputFile.is_open())
    {
    ...
    outputFile << endl;
    outputFile << "----------------------------------" << endl;
    outputFile << "LRU Hit rate: " << lruhit << endl;
    outputFile << "LRU Miss rate: " << lrumiss << endl;
    outputFile << "MRU Hit rate: " << mruhit << endl;
    outputFile << "MRU Miss rate: " << mrumiss << endl;
    outputFile.close();
}
...
int main()
{
    ...
    print1("Cores-info-output.txt", sim.cores[0].num_stalls, sim.cores[1].num_stalls, sim.cores[0].num_Clock_Cycles, sim.cores[1].num_Clock_Cycles, sim.cores[0].num_Instructions, sim.cores[1].num_Instructions, sim.cores[0].forwarding, sim.cores[1].forwarding, sim.c.lrumiss, sim.c.lruhit, sim.c.mrumiss, sim.c.mruhit);
    // For printing the content present in the memory
